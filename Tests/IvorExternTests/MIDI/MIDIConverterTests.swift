...
struct MIDIConverterTests {
}

// MARK: -

extension MIDIConverterTests {
    @Test
    ...
    func roundtrip_tempoMap_accelerando_isStableAfterFirstPass() throws {
        ...
        let sequence1 = try MIDI.Converter().convert(originalWork)
        let work1 = try MIDI.Converter().convert(sequence1)
        ...
        let sequence2 = try MIDI.Converter().convert(work1)
        let work2 = try MIDI.Converter().convert(sequence2)
    ...
    func roundtrip_tempoMap_stepChanges_areIdempotent() throws {
        ...
        let sequence = try MIDI.Converter().convert(originalWork)
        let recoveredWork = try MIDI.Converter().convert(sequence)
